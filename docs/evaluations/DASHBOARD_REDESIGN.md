...
- **표면**: 라이트 회색 배경(#eef0f4 계열) 위 흰 카드, 다크 모드 동시 지원.
- **마크 스펙**: 얇은 마크, 4px 라운드 데이터-엔드, 도넛 세그먼트 2px 간격, 호버 툴팁.

## 구현 단계 (각 단계 = 1 PR, 되돌리기 쉽게)

### 1단계 — 비주얼 셸 (레이아웃·톤) — ✅ 완료 (2026-07-21)
- 디자인 토큰 도입(`--bg`·`--surface`·`--card-radius: 22px`·`--card-shadow`). 배경 회색 위 떠 있는 흰 카드.
- 사이드바 borderless + 떠 있는 nav, 헤더 타이포 조정. prefers-color-scheme 다크 토큰 레이어 추가.
...
### 3단계 — 나머지 화면 정합 (다음)
- 단계·산출물·히스토리·가이드의 세부 색을 토큰으로 정리해 다크에서도 깨지지 않게.

### 4단계 — 반응형·다크모드·접근성 마무리
- 차트 호버 툴팁, 모바일 그리드 점검, 다크 팔레트 컴포넌트까지 검증.
