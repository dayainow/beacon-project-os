...
현재 저장소는 `beacon init → beacon open → project identity → file·Git scan → Beacon signals` 세로 흐름을 구현합니다.
...
브라우저에서 로컬 Dashboard가 열리고, 샘플 데이터가 아니라 지정한 프로젝트 폴더의 이름, 발견한 문서 산출물, 최근 Git 변경과 부족한 기준을 표시합니다. 모든 보완 신호에는 근거, 출처와 다음 행동이 포함됩니다.
